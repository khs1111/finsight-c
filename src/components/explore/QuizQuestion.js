...
export default function QuizQuestion({ current,
  ...
  const titleText =
    (question?.stemMd && String(question.stemMd).trim()) ||
    (question?.question && String(question.question).trim()) ||
    (question?.stem && String(question.stem).trim()) ||
    "문제를 불러오는 중입니다...";
  ...
  const imgCandidates = React.useMemo(() => {
    const primary = (question?.articleImage || question?.image || '').trim();
    const list = [];
    if (primary) list.push(primary);
    list.push(q4ArticlePng); // fallback
    return Array.from(new Set(list.filter(Boolean)));
  }, [question?.articleImage, question?.image]);
