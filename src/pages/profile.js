...
export default function Profile() {
  ...
  const tierImageFor = (tier, urlFromApi) => {
    // 서버에서 상대경로나 프로토콜 생략형을 줄 수 있어 절대 URL로 승격
    const makeAbsolute = (u) => {
      ...
      let origin = '';
      try {
        if (IMAGE_BASE) origin = IMAGE_BASE;
        else {
          const api = new URL(API_BASE, (typeof window !== 'undefined' ? window.location.origin : undefined));
          origin = api.origin;
        }
      } catch (_) { origin = ''; }
  ...
  const hasToken = !!token;
        // Prefer score from user progress; fallback to dashboard if missing
        if (hasUserId) {
          let gotScore = false;
          try {
            const up = await getUserProgress(userId, token);
            const candUP = [
              up?.totalScore, up?.score, up?.points, up?.totalPoints, up?.xp, up?.exp,
              up?.progress?.totalScore, up?.progress?.score, up?.progress?.points,
            ];
            for (const v of candUP) {
              if (v == null) continue;
  ...
  const modalProps = useMemo(() => {
    ...
    return null; 
  }, [modalType]); // modalType이 바뀔 때만 이 값을 다시 계산합니다.
    ...
    <>
      ...
      <div className="profile-content-wrapper">
        ...
        <div className="p-stats-card">
          <div className="stat" aria-label={`점수 ${displayScore}`}>
            ...
            <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label={`점수 배지 ${displayScore}`}>
              ...
              <defs>
                <linearGradient id="paint0_linear_2399_8521" x1="7.75758" y1="11.6364" x2="54.303" y2="58.1818" gradientUnits="userSpaceOnUse">
              ...
              <text
                ...
                style={{
                  fontFamily: "'Roboto', 'Noto Sans KR', AppleSDGothicNeo, 'Apple SD Gothic Neo', MalgunGothic, '맑은 고딕', Arial, sans-serif",
                  fontWeight: 700,
                  fontSize: 18,
                  lineHeight: '21px',
        ...
        <div className="stat">
          ...
          <div className="stat-label">출석완료</div>
        </div>
      </div>
