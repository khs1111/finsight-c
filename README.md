...
4. [주요 기능](#-주요-기능)
5. [패키지 구조](#-패키지-구조)
6. [데이터베이스 설계(프론트 관점)](#-데이터베이스-설계프론트-관점)
7. [API 문서](#-api-문서)
...
### ✅ 완료된 것
- 디자인: 화면별 규격 및 디자인 정상 출력
- Explore: 주제 서브주제 레벨에 맞춘 결과 정상 출력 + 백엔드 호출 정상
- Completion Screen: 화면 규격 및 디자인 완료 + 백엔드 호출 정상
- Auth: 화면 규격 및 디자인 완료 게스트 로그인 + 라우팅 가드 도입 

### 🛠️ 해야 할 것 (우선순위)
1) Profile 배지 데이터 받아오기/표시 정상화 (요약/배지 DTO 매핑 확정)
2) Explore 메인 ‘진행도’ 표시(레벨/서브섹터 진행도 불러와 징검다리/바 반영)
